...
## [2.5.0] - v2.5 UI 重设计（借鉴 OpenRouter）

### 新增
- **明暗双主题**：默认跟随系统，可手动切换并记忆（localStorage），全站生效。
- **设计系统重写**：克制的靛蓝主色（渐变仅用于 Logo/CTA）、双主题 CSS 变量、组件标准化、更精炼的间距/字号。
- **首页数据前置**：实时统计条（可用模型数/渠道数/累计请求）+ 供应商 logo 墙 + 精炼特性区。
- **模型目录页 `/models`**（招牌页）：搜索、按名称/价格排序、¥/$ 切换、每模型调用示例弹窗。数据来自公开端点。
- **后台改为左侧边栏布局**（SaaS 风格），保留全部功能与 JS 逻辑。
- 控制台套用新主题 + 主题切换。
- 新增公开端点 `GET /public/stats`（模型数/渠道数/累计请求，无敏感信息）。

### 变更
- 顶部导航新增「模型」入口。
- 首页/文档/价格/控制台/后台统一新视觉。

### 测试
- 新增 UI/公开端点用例 4 个。累计 93 个全部通过。

## [2.4.0] - v2.4 运营分析与批量操作

### 新增
- **数据分析**页：收入概览（充值/消费/退款/净收入）、模型/用户/渠道消耗排行、按天趋势表、告警区块。
- **收入报表**：按区间聚合 billing_ledger（topup/adjust/consume/refund）+ 净收入。
- **消耗排行**：`/admin/analytics/ranking?by=model|user|channel`（按区间 Top N）。
- **按天趋势**：`/admin/analytics/timeseries`（应用层分桶，补齐空白日期，跨方言一致）。
- **告警查询**：`/admin/alerts` 返回熔断渠道 + 低余额用户。
- **批量操作**：令牌批量启停/删除（`/admin/tokens/batch`）、批量调余额、直接发放充值（grant）。
- 后台令牌页支持多选 + 批量操作条。
- 所有批量/发放操作写审计日志。

### 测试
- 新增分析与批量用例 9 个。累计 89 个全部通过。

## [2.3.0] - v2.3 渠道与账号池增强

### 新增
- **API Key 池精细管理**：单渠道内多 key 的用量/失败统计、单 key 启停（被禁用的 key 路由自动跳过）。
- **渠道连通性测试**：一键对某渠道某模型发极小探测请求，返回每个 key 的 ok/延迟/错误（不计费、不记用量）。
- **模型自动发现**：从上游 `/models` 拉取可用模型列表（OpenAI 兼容渠道），一键复制导入。
- **用户分组聚合视图**：`/admin/groups` 汇总各分组的用户/定价/渠道数。
- 后台渠道行新增「账号池」入口：弹窗查看 key 列表、测试、启停、发现模型。
- key 统计以 sha256 指纹索引，**绝不存明文 key**。

### 数据模型
- `channels` 新增 `key_meta` 列（key 指纹 → 统计/启停），Alembic 迁移 `a1b2c3d4e5f6`。

### 修复/加固
- Router / ChannelService 的上游 key 解密改用应用注入的 `crypto_secret`（不再依赖全局单例），
  多实例/测试环境更稳健。

### 测试
- 新增渠道账号池用例 7 个。累计 80 个全部通过。
