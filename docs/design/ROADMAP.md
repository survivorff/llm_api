...
- [ ] 充值订单系统（orders）+ 异步对账 worker
- [ ] 加密货币充值适配器（USDT-TRC20 / 聚合网关）
- [ ] 微信 / 支付宝扫码充值适配器（易支付聚合 + 官方商户可选）
- [ ] 兑换码充值
- [ ] 支付适配器接口标准化（可插拔，部署者自选）
...
- [ ] 邮箱注册/登录 + 邮件验证
- [ ] OAuth 登录（GitHub / LinuxDO / Telegram）
- [ ] 用户自助：用量仪表盘、账单明细、令牌自助管理
- [ ] 多语言（中/英）
...
- [ ] 安装向导（首次启动引导初始化）
- [ ] 系统设置热更新、公告、模型价格表可视化管理
- [ ] Prometheus 指标 + Grafana 面板样例
- [ ] 审计日志 + 日志保留策略
- [ ] 完整文档站 + 一键部署脚本 + CI/CD
- [ ] License 与合规声明定稿
